...
User = get_user_model() # find the project's user model

class LoginViewTest(TestCase):

    @patch('accounts.views.authenticate') #specify object to mock out
    def test_calls_authenticate_with_assertion_from_post(
        self, mock_authenticate 
    ):
        mock_authenticate.return_value = None #configure mock behavior
    ...
    def test_gets_logged_in_session_if_authenticate_returns_a_user(
    	...
    	self.assertEqual(self.client.session[SESSION_KEY], str(user.pk)) #check uid is assoc. with session
    ...
    def test_does_not_get_logged_in_if_authenticate_returns_None(
    	...
    	self.client.post('/accounts/login', {'assertion': 'a'})
    	self.assertNotIn(SESSION_KEY, self.client.session) #user should not be authenticated
